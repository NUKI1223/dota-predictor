...
from dota_predictor.features.roster import ROSTER_COLS
...
def train_and_evaluate_gbdt(
    ...
    roster_cols = [c for c in ROSTER_COLS if c in df.columns and df[c].notna().any()]
    configs = [
        ...
        ("gbdt +rosters", FEATURE_COLS + DRAFT_COLS + meta_cols + ban_cols + roster_cols),
        ("gbdt +bag-of-heroes",
         FEATURE_COLS + DRAFT_COLS + meta_cols + ban_cols + roster_cols + hero_cols),
    ]
    if roster_cols:
        best_name = "gbdt +rosters"
    elif ban_cols:
        best_name = "gbdt +bans"
    elif meta_cols:
