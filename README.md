...
- [x] v7: live-трекер (`python -m dota_predictor.live [--watch 60]
      ...
      предматчевая вероятность карты + поправка на текущее преимущество
      по золоту (эвристика, помечена как ненатренированная); сравнение
      с букмекерской линией через `--odds`.
